import { Platform, StyleSheet, Text, View, Image, TouchableOpacity, ScrollView, NativeSyntheticEvent, NativeScrollEvent, ActivityIndicator } from 'react-native';
...
import { FETCH_COMPLETED_TICKET_CARDS, FETCH_UPCOMING_TICKET_CARDS } from '../../api/Request';
...
const ScreenTwo = () => {
    ...
    const getCompletedTickets = async () => {
        try {
            const response = await getRequestPlusHeaders(`${FETCH_COMPLETED_TICKET_CARDS}`, stationId);
            if (response) {
                console.log('success');
                console.log(response);
                const mappedTickets: TicketCardProps[] = response.map((ticket: TicketResponse) => ({
